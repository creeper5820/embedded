...
class CanServer {
public:
    using HandleFunction = void (*)(CAN_RxHeaderTypeDef&, const uint8_t*);
...
public:
        ...
        : hcan_(hcan)
    {
        ...
        handle_ = [](CAN_RxHeaderTypeDef& header, const uint8_t data[8]) -> void {
            (void)header;
            (void)data;
        };
    }

    void set_filter(const CAN_FilterTypeDef& filter)
    {
        filter_ = filter;
    }

    void set_handle(const HandleFunction& handle)
    {
        handle_ = handle;
    }
